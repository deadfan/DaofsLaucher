...
import {
  ...
  updateCurseChannelLang,
  updateCurseChannelBackground
...
const PersonalDataContainer = styled.div`
  ...
  border: solid ${props => props.theme.palette.grey[50]} 2px;
`;
...
const CustomBackGroundContainer = styled.div`
  display: flex;
  flex-direction: column;
  width: 100%;
  height: 140px;
...
const General = () => {
  ...
  useEffect(() => {
    ...
    ipcRenderer
      ...
      .then(appData =>
        fsa
          .readFile(path.join(appData, 'daofslauncher_next', 'rChannel'))
  ...
  const changeDataPath = async () => {
    ...
    const appDataPath = path.join(appData, 'daofslauncher_next');
    ...
    <MyAccountPrf>
      <PersonalData>
        ...
        <PersonalDataContainer>
          {profileImage ? (
            <ProfileImage src={`https://crafatar.com/renders/body/${dashUuid(currentAccount.selectedProfile.id)}?scale=3&default=MHF_Steve&overlay`} />
          ) : (
            <ImagePlaceHolder />
          )}
          <div
...
<Hr />
      ...
      <ParallelDownload>
        ...
        <Select
          css={`
            ...
            text-align: start;
          `}
          onChange={e => dispatch(updateCurseChannelLang(e))}
          value={curseChannelLang}
          virtual={false}
        >
      ...
      <Title>
        Background &nbsp; <FontAwesomeIcon icon={faFire} />
      </Title>
      <ParallelDownload>
        <p
          css={`
        ...
        <Select
          css={`
            ...
            text-align: start;
          `}
          onChange={e => dispatch(updateCurseChannelBackground(e))}
          value={curseChannelBackground}
          virtual={false}
        >
