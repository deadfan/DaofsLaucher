           ...
           {t('modaljavasetup.description')}
            ...
            >
              <div>
                ...
                >
				{t('modaljavasetup.automaticsetup')}
                </Button>
              </div>
              <div>
                <Button
        ...
        {state => (
          <SecondStep state={state}>
            <div
              ...
              `}
            >
              {choice === 0 ? t('modaljavasetup.automatic') : t('modaljavasetup.manual')} {t('modaljavasetup.setup')}
            </div>
            {choice === 0 ? (
              <AutomaticSetup />
            ) : (
  ...
  return (
    ...
    >
      <div
        ...
        `}
      >
	  {t('modaljavasetup.manualdescription')}
      </div>
      <div
        css={`
          width: 100%;
      ...
      >
        <Input
          placeholder={t('modaljavasetup.selectjava')}
      ...
      >
		...
		{t('modaljavasetup.backtoautomatic')}
        </Button>
        <Button
          type="danger"
          disabled={javaPath === ''}
        ...
        >
		{t('modaljavasetup.continuecustomjava')}
        </Button>
      </div>
    </div>
  );
...
const AutomaticSetup = () => {
  ...
  const [currentStep, setCurrentStep] = useState(t('modaljavasetup.downloadingjava'));
  ...
  const installJava = async () => {
	...
	const backgroundBaseFolder = path.join(userData, 'background');
    ...
    setCurrentStep(t('modaljavasetup.extracting', { namber: '1'}, '/ ${totalSteps}'));
    ...
    if (process.platform !== 'win32') {
      ...
      setDownloadPercentage(null);
      await new Promise(resolve => setTimeout(resolve, 500));
      setCurrentStep(t('modaljavasetup.extracting'), { namber: '2'}, '/ ${totalSteps}');
    ...
    const directoryToMove =
      process.platform === 'darwin'
        ? path.join(tempFolder, `${releaseName}`, 'Contents', 'Home')
        : path.join(tempFolder, `${releaseName}`);
    await fse.move(directoryToMove, path.join(javaBaseFolder, version));

    await fse.remove(path.join(tempFolder, `${releaseName}`));

    const ext = process.platform === 'win32' ? '.exe' : '';

    if (process.platform !== 'win32') {
      const execPath = path.join(javaBaseFolder, BaseFolder, version, 'bin', `java${ext}`);

      await promisify(exec)(`chmod +x "${execPath}"`);
      await promisify(exec)(`chmod 755 "${execPath}"`);
    }
	
    const directoryToMoveBackground =
      process.platform === 'darwin'
        ? path.join(tempFolder, `backgrounds`)
        : path.join(tempFolder, `backgrounds`);
    await fse.move(directoryToMoveBackground, path.join(backgroundBaseFolder, 'default'));

    await fse.remove(path.join(tempFolder, `backgrounds`));
	
	
    dispatch(updateJavaPath(null));
    setCurrentStep(t('modaljavasetup.javaisready'));
    ipcRenderer.invoke('update-progress-bar', -1);
    setDownloadPercentage(null);
