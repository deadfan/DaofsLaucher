...
export function initManifests() {
  return async (dispatch, getState) => {
    ...
    const [fabric, java, categories] = await Promise.all([
      reflect(getFabricVersions()),
      reflect(getJavaManifestVersions()),
      reflect(getAddonCategoriesVersions())
    ]);

    if (fabric.e || java.e || categories.e) {
      console.error(fabric, java, categories);
    }
  ...
  return async (dispatch, getState) => {
    ...
    for (let i = 0; i < accounts.length; i += 1) {
      ...
      try {
        ...
        console.error(t('error.failedtovalidate') `${accounts[i].selectedProfile.id}`);
      }
    }
    if (!found) {
...
export function login(username, password, redirect = true) {
  return async (dispatch, getState) => {
    ...
    if (!username || !password) {
      throw new Error(t('error.nouserpassword'));
    }
    try {
      let data = null;
      try {
		...
		data.accountType = ACCOUNT_DARK;
      } catch (err) {
        console.error(err);
        throw new Error(t('error.invaliduserpasswrod'));
      }

      if (!data?.selectedProfile?.id) {
        throw new Error(t('error.dontbuygame'));
      }
    ...
    if (Date.now() >= msExpiresAt) {
      ...
      try {
        ...
        try {
          ...
          console.error(error);
          throw new Error(t('error.refmicrotoken'));
        }
        ...
        try {
          ...
          console.error(error);
          throw new Error(t('error.loggingstudio', {studio : 'Xbox Live'}));
        }
        ...
        try {
          ...
          console.error(error);
          throw new Error(t('error.loggingstudio', {studio : 'Minecraft'}));
        }
      ...
      try {
        ...
        console.warn(t('error.couldnotskin'));
      }
    }
  };
}
...
export function loginWithAccessToken(redirect = true) {
  return async (dispatch, getState) => {
    ...
    try {
      ...
      } else if (error.message === t('error.networkerror')) {
        if (redirect) {
          dispatch(push('/home'));
        }
      }
  ...
  return async (dispatch, getState) => {
    ...
    try {
      ...
      try {
        ...
        console.error(error);
        throw new Error(t('error.loggingstudio', {studio : 'Microsoft'}));
      }
      ...
      try {
        ...
        throw new Error(t('error.erroroccurredmineprofile'));
      }
    ...
    const update = async () => {
      ...
      const writeFileToDisk = async (content, tempP, p) => {
            ...
            fss.write(
              ...
              (err1, bytesWritten, writtenBuffer) => {
                ...
                if (
                  ...
                  reject(new Error(t('error.contentcorruped')));
                }
        ...
        if (readBuff.every(v => v === 0)) {
          throw new Error(t('error.corruptedfile'));
        }
...
export const getAppLatestVersion = async () => {
  ...
  try {
    const rChannel = await fs.readFile(
      path.join(appData, 'daofslauncher_next', 'rChannel')
    );
