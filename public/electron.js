...
if (!app.isDefaultProtocolClient('daofslauncher')) {
  app.setAsDefaultProtocolClient('daofslauncher');
}
...
app.setPath('userData', path.join(app.getPath('appData'), 'daofslauncher_next'));
...
function createWindow() {
  ...
  const trayMenuTemplate = [
    ...
    {
      label: 'Exit',
      click: () => process.exit(42)
    }
  ];

  const trayMenu = Menu.buildFromTemplate(trayMenuTemplate);
  tray.setContextMenu(trayMenu);
  tray.setToolTip('DaofsLauncher');
